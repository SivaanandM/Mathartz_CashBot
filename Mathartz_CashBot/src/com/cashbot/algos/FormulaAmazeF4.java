...
public class FormulaAmazeF4 {
	...
	public void FormulaAmazeDriver(String [] ids, double LTP, Date LTT)
	{
		try
		{
			...
			for (String id : ids) 
			{
				...
				finput = listcom.getFormulaDataByID(identity,  CommonObjects.GlobalAmazeF4);
				if (finput != null)
				{
					if (!finput.getIsend())
					{
	...
	public void AssignvVariables()
	{
		try
		{
			...
			fvalue = listcom.getAmazeValuesByID(identity, CommonObjects.GlobalAmazeValuesF4);
			c = fvalue.getc();
			lc = fvalue.getlc();
			r = fvalue.getr();
