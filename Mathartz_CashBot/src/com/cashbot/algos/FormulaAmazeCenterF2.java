...
public class FormulaAmazeCenterF2 {
	...
	public void FormulaAmazeDriver(String [] ids, double LTP, Date LTT)
	{
		try
		{
			...
			for (String id : ids) 
			{
				...
				finput = listcom.getFormulaDataByID(identity,  CommonObjects.GlobalAmazeF2);
				if (finput != null)
				{
					if (!finput.getIsend())
					{
	...
	public void AssignvVariables()
	{
		try
		{
			...
			fvalue = listcom.getAmazeCenterValuesByID(identity, CommonObjects.GlobalAmazeCenterValuesF2);
			c = fvalue.getc();
			lc = fvalue.getlc();
			r = fvalue.getr();
